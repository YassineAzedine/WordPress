...
if (is_admin()) {
     ...
     */
    $my_meta = new Meta_Store_Meta_Box($config);

    /*
     * Add fields to your meta box
     */

    $my_meta->openTab($prefix . 'genera_setting', array('name' => 'General Settings', 'icon' => 'dashicons-admin-generic dashicons'));

    //text field
    $my_meta->addText($prefix . 'text_field_id', array('name' => 'My Text ', 'std' => 'fsdfsfs'));
    //textarea field
    $my_meta->addTextarea($prefix . 'textarea_field_id', array('name' => 'My Textarea '));
    //checkbox field
    $my_meta->addCheckbox($prefix . 'checkbox_field_id', array('name' => 'My Checkbox ', 'desc' => 'This is my Checkbox '));

    $my_meta->addBackground($prefix . 'background_field_id', array('name' => 'My Image '));

    $my_meta->addImageRadio($prefix . 'image_radio_field_id', array(
        'radiokey1' => get_template_directory_uri() . '/inc/theme-options/images/sidebar-layouts/default-sidebar.jpg',
        'radiokey2' => get_template_directory_uri() . '/inc/theme-options/images/sidebar-layouts/left-sidebar.jpg',
        'radiokey3' => get_template_directory_uri() . '/inc/theme-options/images/sidebar-layouts/right-sidebar.jpg',
            ), array(
        'name' => 'My Radio Filed',
        'std' => array('radiokey3')
    ));
    ...
    $my_meta->openTab($prefix . 'other_setting', array('name' => 'Other Settings', 'icon' => 'dashicons-admin-generic dashicons'));
    //select field
    ...
    //Image field
    $my_meta->addImage($prefix . 'image_fiesld_id', array('name' => 'My Image '));
     ...
     */
    $my_meta2 = new Meta_Store_Meta_Box($config2);
    ...
    $my_meta2->addAplhaColor($prefix . 'color_alpha_fieled_id', array('name' => 'My Color', 'std' => '#FFF'));
    //wysiwyg field
    ...
    $repeater_fields[] = $my_meta2->addPosts($prefix . 'posts_field_id', array('post_type' => 'post'), array('name' => 'My Posts '), true);
    $repeater_fields[] = $my_meta2->addAplhaColor($prefix . 'color_alpha_fieled_id', array('name' => 'My Color', 'std' => '#FFF'), true);
    //wysiwyg field
    $repeater_fields[] = $my_meta2->addWysiwyg($prefix . 'wysiwyg_field_id', array('name' => 'My wysiwyg Editor '), true);
    $repeater_fields[] = $my_meta2->addDate($prefix . 'date_field_id', array('name' => 'My Date '), true);
    //Time field
    $repeater_fields[] = $my_meta2->addTime($prefix . 'time_field_id', array('name' => 'My Time '), true);
    //Color field
    $repeater_fields[] = $my_meta2->addColor($prefix . 'color_field_id', array('name' => 'My Color '), true);
    /*
     * Then just add the fields to the repeater block
     */
    //repeater block
    $my_meta2->addRepeaterBlock($prefix . 'repeater_field', array(
        'inline' => false,
    ...
    $my_meta2->addCondition('conditinal_fields', array(
        'name' => __('Enable conditinal fields? ', 'meta-store'),
        'desc' => __('<small>Turn ON if you want to enable the <strong>conditinal fields</strong>.</small>', 'meta-store'),
     ...
     */
    $my_meta3 = new Meta_Store_Meta_Box($config3);
